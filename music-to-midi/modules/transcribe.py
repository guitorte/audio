"""Stem → MIDI transcription (single stem).
...
Adapted from the repo's ``stem-to-midi`` package so this project is
self-contained. Heavy backends (basic_pitch, adtof_pytorch) are imported
lazily, so the module imports fine without them installed.
"""
...
# Per-instrument Basic Pitch tuning + engine routing.
STEM_PRESETS = {
    "bass": {
        "engine": "basic_pitch",
        "onset_threshold": 0.5,
...
# General MIDI program assigned to each pitched stem when merging tracks.
# Drums are not pitched — they go to channel 10 (is_drum) regardless.
...
def canonical_stem_type(name: str) -> Optional[str]:
    ...
    Returns one of STEM_PRESETS keys, or None if nothing matches. Matching is
    substring-based and case-insensitive so Demucs outputs ('vocals.wav') and
    looser names ('lead_vocal', 'Kim_Vocal_2') both resolve.
    """
    stem = os.path.splitext(os.path.basename(name))[0].lower()
    # Demucs two-stems mode emits the complement as 'no_<stem>' (e.g.
    # 'no_vocals' = instrumental). Treat any such complement as 'other' before
    # the substring checks below, so 'no_vocals' doesn't match 'vocals'.
